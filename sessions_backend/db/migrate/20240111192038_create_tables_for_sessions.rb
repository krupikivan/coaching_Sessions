...
class CreateTablesForSessions < ActiveRecord::Migration[6.1]
  def change
    ...
    %w[robert james john michael william david richard charles joseph thomas].each do |name|
      coach = Coach.new
      coach.name = name
      coach.coach_hash_id = coach_generate_unique_hash
      coach.save
    end
    %w[mary patricia jennifer linda elizabeth barbara susan jessica sarah
       karen].each do |name|
      client = Client.new
      client.name = name
